...
// Pinta um trecho dentro do texto extraído. Procura o trecho exato
// (case-insensitive) e o envolve com highlight amarelo.
...
export default function ClauseViewer({ nomeArquivo, risco, totalPaginas, texto }) {
  return (
    <Card className="overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-ink-400">
              Cláusula {risco?.numero_clausula} · Pág. {risco?.pagina} / {totalPaginas}
            </p>
          </div>
        </div>
      ...
      <div className="px-6 py-6">
        <div className="whitespace-pre-line text-[15px] leading-relaxed text-ink-700">
          {highlight(texto, risco?.trecho_clausula)}
        </div>
        ...
        <div className="mt-5 flex items-start gap-2 rounded-md bg-amber-50/80 px-3 py-2 text-sm text-amber-800 ring-1 ring-amber-200">
          ...
          <span>Trecho sem teto financeiro — diagnóstico no painel →</span>
        </div>
      </div>
    </Card>
  );
